...
# Debian-only
describe 'supervisor::in_systemd' do
  let(:chef_run) do
...
describe 'supervisor::in_systemd' do
  ...
  it 'configures supervisord service' do
    expect(chef_run).to create_template("/etc/systemd/system/supervisord.service")
  end
  ...
  it 'uses systemd to start supervisord' do
    expect(chef_run).to run_execute('systemctl start supervisord.service')
  end
end
...
describe 'supervisor::in_systemd' do
  ...
  it 'configures supervisor service' do
    expect(chef_run).to create_template("/etc/systemd/system/supervisor.service")
  end
  ...
  it 'uses systemd to enable supervisor' do
    expect(chef_run).to run_execute('systemctl enable supervisor.service')
  end

  it 'uses systemd to start supervisor' do
    expect(chef_run).to run_execute('systemctl start supervisor.service')
  end
end